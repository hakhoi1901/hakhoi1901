## Hi, I'm Khôi (@Hakhoi1901)

I am a second-year Computer Science student at **VNU-HCM University of Science**. My work revolves around core system architecture, low-level mechanics, and from-scratch algorithmic implementations.

---

### 🔭 Current Focus & Learning
I prioritize understanding the "why" and the internal nature of systems rather than just using high-level frameworks.
* **System Architecture & Backend:** C++, C#, ASP.NET Core, Object-Oriented Design, Backend Logic strictly separated from UI/UX.
* **Algorithms & Optimization:** Constraint Satisfaction Problems (CSP), Genetic Algorithms / Neuroevolution.

---

### 🛠️ Projects
Here are some projects where I apply my knowledge to solve practical problems:

* **[UStudy](https://github.com/hakhoi1901/ustudy)**
    * **Description:** A utility tool (bookmarklet) designed to enhance the user experience on the university's training portal, featuring academic progress analysis and course recommendations.
    * **Stack:** JavaScript, DOM Manipulation.

* **[Exoplanet Mass Prediction](https://github.com/hakhoi1901/Exoplanet-mass-prediction)**
    * **Description:** 
        * Engineered a robust regression pipeline to predict exoplanet masses, utilizing MICE for data imputation, Winsorization for outlier handling, and VIF analysis to eliminate multicollinearity.
        * Optimized Ridge and Lasso models via cross-validation, performing rigorous residual analysis and Hat matrix diagnostics to define model boundaries and identify highly influential observations.    
    * **Stack:** Python, Statistical Modeling, Ridge/Lasso} \hfill \textit{2026}

* **[Smart Course Scheduler](https://github.com/hakhoi1901/Smart-Course-Scheduler)**
    * **Description:** An automated scheduling tool designed to solve Constraint Satisfaction Problems (CSP), aiming to resolve time conflicts while respecting student preferences.
    * **Stack:** C++/Python, Optimization Algorithms.

---

### 🤝 Collaborative Work & Core Logic

* **[TasteMap](https://github.com/EndlessMelody/tastemap)** | *Core Backend Logic*
    * **Boundary:** Strictly handled backend mathematics and API architecture; UI/UX decoupled.
    * **Contribution:** Engineered the "Interactive Feed" API, utilizing Vector Space Models (VSM) to compute mathematical scoring logic based on user preference vectors.
    * **Stack:** Python, FastAPI, VSM.
  
---

### 📫 Contact
* **Email:** Hakhoi.contact@gmail.com
